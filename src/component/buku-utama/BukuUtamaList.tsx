...
export default function BukuUtamaList() {
  ...
  const handleDelete = (id: string) => {
    if (window.confirm('Apakah Anda Yakin Menghapus Data ini?')) {
      apiClient.delete(`/buku-utama/${id}`)
      .then(() => {
  ...
  const columns: TableColumn<BukuUtamaDto>[] = [
    { key: 'tanggal', label: 'Tanggal Transaksi', sortable: true, render: (item) => new Date(item.tanggal).toLocaleDateString('id-ID') },
    { key: 'kodeTransaksi', label: 'Kode Transaksi' },
    {key: 'kodeAkun', label: 'Kode Akun', render: (item) => item.kodeAkun || 'N/A'},
    { key: 'namaAkun', label: 'Nama Akun', render: (item) => item.namaAkun || 'N/A'},
    { key: 'namaKegiatan', label: 'Nama Kegiatan', render: item => item.namaKegiatan|| 'N/A' },
    {key: 'kodeKegiatan', label: 'Kode Kegiatan', render: item => item.kodeKegiatan || 'N/A'},
    ...
    { key: 'rekeningTujuan', label: 'Rekening Tujuan' },
    {
      ...
      label: 'Trace Number',
      render: (item) => (
        <span title={item.traceNumber} style={{ fontSize: '12px', color: 'black' }}>
    ...
    },
    {
      key: 'nominalKeluar',
      label: 'Uang Keluar',
      render: (item) => `Rp${(item.nominalKeluar ?? 0).toLocaleString('id-ID')}`
    },
    {
      key: 'saldoCash',
      label: 'Saldo Cash',
    ...
    },
    {
      ...
      render: (item) => (
        ...
        <div className='action-buttons'>
          ...
          <button className="btn-delete" onClick={() => handleDelete(item.traceNumber)}>HAPUS</button>
        </div>
      )
    }
  ];
  ...
  return (
    <div className='buku-utama-container'>
      {/* <h2>Buku Utama</h2> */}
      <h2><Link to="/" style={{ color: 'inherit', textDecoration: 'none' }}>Buku Utama</Link></h2>
      ...
      <div className="filter-section">
        ...
        <div className="filter-controls">
          ...
          <button onClick={handleCreate} className="btn-tambah" disabled={isLoading}>
            + Tambah Data
          </button>
        </div>
      </div>
